...
/**
 * A MOSS File System Simulator program which uses the simulator classes to
...
public class mkfs {
	...
	public static void main(String[] argv) throws Exception {
		...
		/*
		 * blocks = super_blocks + free_list_blocks + inode_blocks + data_blocks
		 * 
		 * We need one block for the superblock. super_blocks = 1
		 * 
		 * We need one bit in the free list map for each data block. free_list_blocks =
		 * ( data_blocks + block_size * 8 - 1 ) / ( block_size * 8 )
		 * 
		 * ??? Is this the correct number of inodes?
		 * 
		 * At worse, there will be only directory entries and empty files. In other
		 * words, we might need as many inodes as we have blocks. inode_blocks = (
		 * data_blocks + block_size / inode_size - 1 ) / ( block_size / inode_size )
		 * 
		 * Then:
		 * 
		 * blocks = super_blocks + ( data_blocks + block_size * 8 - 1 ) / ( block_size *
		 * 8 ) + ( data_blocks + block_size / inode_size - 1 ) / ( block_size /
		 * inode_size ) + data_blocks
		 * 
		 * We then seek the maximum number of data blocks where the total number of
		 * blocks is less than or equal to the number of blocks available. We use a
		 * binary searching technique in the following algorithm.
		 */
		...
		while (lo <= hi) {
			 ...
			 * 
			 * System.out.println( "lo: " + lo + " hi: " + hi ) ; System.out.println(
			 * "block_size: " + block_size ) ; System.out.println( "blocks: " + blocks ) ;
			 * System.out.println( "free_list_blocks: " + free_list_blocks ) ;
			 * System.out.println( "inode_blocks: " + inode_blocks ) ; System.out.println(
			 * "data_blocks: " + data_blocks ) ; System.out.println( "block_total: " +
			 * block_total ) ; System.out.println() ;
			 */
